...
bool : boolian이라고도 합니다. 참(True) 또는 거짓(False) 2개의 값만 가집니다.
        파이썬에서는 True와 False의 첫 글자를 대문자로 사용합니다.
str : 문자열(string) -> 흔히 말하는 텍스트로 '' 또는 ""로 감싸야만 합니다.
        ''처럼 내부에 문자가 없어도 문자열이 생성이 된 것으로 간주합니다.
'''

name = '서준호' #str 문자열
age = '20' #str 문자열(작은 따옴표로 감싼 숫자도 문자열입니다.)
age = 20 #int 정수(소수점이 없는 수)
weight = 80.5 #float 실수(소수점이 있는 수)
...
#문자열을 정수나 실수로 변환할 때는 숫자의 형식을 따라야만 합니다.
...
print(int(light_on))
print(float(light_on))
number = 1.012_345_678_901_23456789 #소수점 19자리 기록
